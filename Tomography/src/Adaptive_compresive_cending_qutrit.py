...
def pl_fid_max_min(x, fidelity_mean_1, fidelity_mean_2, title=None):
  ...
  ax1.tick_params(axis='both', direction='in')
  ax1.set_title("График зависимости Fidelity между" + r" $\rho_{min}$ и" + "\n начальным состоянием от количества измерений")
  ax1.set_xlabel('Количество измерений')
  ax1.set_ylabel(r'$S_{\mathrm{cvx}}$') 
  ax1.set_xticks(x)
  ax1.set_ylim(-0.1, 1.1)
  ...
  ax2.tick_params(axis='both', direction='in')
  ax2.set_title("График зависимости Fidelity между" + r" $\rho_{max}$ и " + "\n начальным состоянием от количества измерений")
  ax2.set_xlabel('Количество измерений')
  ax2.set_ylabel('Fidelity')
  ax2.set_ylim(-0.1, 1.1)
  ax2.set_xticks(x)
